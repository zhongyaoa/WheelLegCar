...
int16 turn_duty_max = 600;   // 原300→600，提升转向力矩（占总输出10000的6%）
...
void pit_call_back(void)
{
    ...
    // ── 循迹控制内环：每 5ms，仅乘加运算（KP*heading_err - KD*gyro_z → turn_diff_ext）
    // 与角度环同频，在同一中断上下文写 turn_diff_ext，car_motor_control() 下次读取
    if(sys_times % 5 == 0)
    {
        if(inav_active) ins_tracker_ctrl_update();
    }

    // ── 循迹导航外环：每 10ms，含 sqrtf/atan2f/cosf/sinf
    // 计算距离、目标方位角、前瞻航点切换、自适应速度，结果写入共享变量
    if(sys_times % 10 == 0)
    {
        if(inav_active) ins_tracker_nav_update();
    }
}
